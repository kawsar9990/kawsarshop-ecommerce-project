...
<Link href={`/product/${item.id}`}>
  ...
  <div className="p-2 mb-2">
      <button className="bg-transparent hover:bg-black hover:text-white hover:outline-0 outline-2 text-center rounded-md text-[#E2136E] cursor-pointer outline-red-600  p-1 w-full">
        <Link href={`/product/${item.id}`}  onClick={()=> handleLoading(item.id)}>
...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-4">
 ...
 <div className="flex flex-col items-start justify-between lg:justify-center p-3 w-full lg:w-1/2">
      ...
      <button
        ...
        >
          <Link href={`/product/${item.id}`} onClick={() => handleLoading(item.id)} className="flex flex-row gap-3 justify-center">
            <p>
              <FontAwesomeIcon icon={faCartShopping} />
            </p>
            <p>Add To Cart</p>
