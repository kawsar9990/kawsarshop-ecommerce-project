...
import { registerUser } from "../../../src/api/apiService";
...
const handleSubmit = async (e) => {
    ...
    if (!captchaToken) {
        notify.error("Please verify that you are human!");
        return;
    }
...
<label className='text-orange-500 cursor-pointer text-[12px] font-bold uppercase'>Date of birth:</label>
<div className="flex gap-2 mt-1 cursor-pointer">
<div className="relative w-1/3">
    <select 
        className={`w-full cursor-pointer p-3 appearance-none border rounded-md bg-white text-[13px] outline-none transition ${errortext.dob ? 'border-red-500' : 'border-orange-600'}`}
        value={dob.day}
        onChange={(e) => setDob({ ...dob, day: e.target.value })}>
        <option value="">Day</option>
        {days.map(d => <option key={d} value={d}>{d}</option>)}
    </select>
    <FontAwesomeIcon icon={faChevronDown} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 text-[12px] pointer-events-none" />
</div>
<div className="relative w-1/3">
    <select 
        className={`w-full p-3 cursor-pointer appearance-none border rounded-md bg-white text-[13px] outline-none transition ${errortext.dob ? 'border-red-500' : 'border-orange-600'}`}
        value={dob.month}
...
<div className="flex gap-2">
    <input type="checkbox" id="newsl" name="newsl" checked={newsletter}
...
<Turnstile 
siteKey="0x4AAAAAACwaQCsOd6jbsyS8"
