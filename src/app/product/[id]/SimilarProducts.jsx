...
const related = products.filter(item => {
    ...
    const isCateTitleMatch = item.catetitle && item.catetitle === product.catetitle;
    const isTitleMatch = (item.title && item.title === product.title) || 
                         (item.name && item.name === product.name);
    return isCateTitleMatch || isTitleMatch;
}).slice(0, 6);
...
<div className="p-5">
<p className="text-[20px] font-semibold">Similar Product</p>
</div>
