...
export default function ReturnInitiateTableView({ orders, returnHistory = [] }) {
...
return (
<div className="hidden md:block space-y-6" style={{userSelect: "none"}}>
{orders.map((order) => {
...
<div key={`${order._id}-${item._id || index}`} className="border border-gray-200 rounded-lg bg-white overflow-hidden shadow-sm">      
...
<span className="text-[12px] font-bold text-emerald-600 capitalize bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100">
  {order.orderStatus}
</span>
</div>
...
<div className="w-[10px] h-[1.5rem]"></div>
) : isExpired && finalReturnStatus === 'Not Applied' ? (
<span className="text-[10px] font-bold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-md border border-gray-200 inline-block cursor-not-allowed">
  Return Period Expired
</span>
) : finalReturnStatus === 'Not Applied' ? (
  <button
  onClick={() => openApplyReturnModal(order._id, item._id)}
  className="bg-emerald-600 text-white rounded-md cursor-pointer px-4 py-1.5 text-[10px] font-bold hover:bg-emerald-700 transition-all shadow-sm">
    Apply Return
  </button>
) : finalReturnStatus === 'Pending' || finalReturnStatus === 'Return Requested' ? (
  <button
  onClick={() => openCancelReturnModal(order._id, item.product)}
  className="bg-amber-600 text-white rounded-md cursor-pointer px-4 py-1.5 text-[10px] font-bold hover:bg-amber-700 transition-all shadow-sm">
   Cancel Return
  </button> 
) : (
  <span className="text-[10px] font-bold text-gray-400 bg-gray-100 px-3 py-1.5 rounded-md border border-gray-200">
    {finalReturnStatus}
  </span>
)} 
</div>
...
<Modal open={step === 1} onClose={closeModal} sx={{ zIndex: 99999, userSelect: "none" }}>
<Box sx={modalStyle}>
...
<Modal open={step === 2} onClose={closeModal} sx={{ zIndex: 9999999, userSelect: "none" }}>
<Box sx={modalStyle}>
...
<Modal open={step === 6} onClose={closeModal} sx={{ zIndex: 9999999, userSelect: "none" }}>
<Box sx={modalStyle}>
  <div className="flex justify-between items-center mb-3">
...
>
    ...
    <div>
      <h4 className="text-[14px] font-bold text-slate-800 leading-tight">{method.title}</h4>
      <p className="text-[11px] text-slate-600 mt-1 leading-normal">{method.description}</p>
    </div>
  </div>
...
<Modal open={step === 7} onClose={closeModal} sx={{ zIndex: 9999999, userSelect: "none" }}>
<Box sx={modalStyle}>
